...
PackageManager.publish = function(params) {
    if (params.length == 3) {
        ...
        console.log("       -p                                    发布当前目录下的所有自定义包");
        console.log("       -P                                    发布当前目录下的所有项目");
        console.log("       -p  username:package                  发布当前目录下的自定义包和项目");
        console.log("       -P  project                           发布当前目录下的项目\n");
        return;
    }
}
...
function publishTo(args) {
    ...
    if (args.length < 1) {
        ...
        for (var i = 0; i < trees.length; i++) {
            ...
            if (!name) {
                // console.log(name);
                if (fileName.indexOf("depot.json") >= 0 && type == "-p") {
                    // console.log('publish ' + type + " " + fileName);
                    var config = require(path.resolve(".aics") + "/" + fileName);
                    publishToDeopt(config, path.resolve(".aics") + "/" + fileName);
                }
                if (fileName.indexOf("project.json") >= 0 && type == "-P") {
                    // console.log('publish ' + type + " " + fileName);
                    var config = require(path.resolve(".aics") + "/" + fileName);
                    publishProject(config);
                }
            } else {
                if (fileName.indexOf("depot.json") >= 0 && type == "-p" && fileName.indexOf(name) >= 0) {
                    var config = require(path.resolve(".aics") + "/" + name + ".depot.json");
                    publishToDeopt(config, path.resolve(".aics") + "/" + name + ".depot.json");
                }
                if (fileName.indexOf("project.json")>=0 && type == "-P" && fileName.indexOf(name) >= 0) {
