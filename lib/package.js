...
function publishTo(args) {
    ...
    if (args.length<1) {
        ...
        for (var i = 0; i < trees.length; i++) {
            ...
            // var config = Tools.projectConfig(".aics/" + fileName);
            // if (configCheck(config)) {
                ...
                if (fileName.indexOf("project") >= 0) {
                    ...
                    var config = require(path.resolve(".aics")+"/aics-project.json");
        ...
        for (var i = 0; i < trees.length; i++) {
            ...
            // console.log("读取配置文件: .aics/" + fileName);
            // var config = Tools.projectConfig(".aics/" + fileName);
            // if (configCheck(config)) {
                if (!name) {
                    ...
                    var config = require(path.resolve(".aics")+"/aics-project.json");
                        publishToDeopt(config, ".aics/" + fileName);
                    }
                    if (fileName == "aics-project.json" && type == "--project") {
                        console.log('publish ' + type + " " + fileName);
                    var config = require(path.resolve(".aics")+"/aics-project.json");
                        publishProject(config);
                    }
                } else {
                    // console.log('publish '+type+' by '+name);
                    if (fileName.indexOf("depot") >= 0 && type == "--package" && fileName.indexOf(name) >= 0) {
                        publishToDeopt(config, ".aics/" + name + ".depot.json");
                    }
                    if (fileName == "aics-project.json" && type == "--project" && fileName.indexOf(name) >= 0) {
                        publishProject(config);
                    }
                }
            // }
