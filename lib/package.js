...
PackageManager.add = function(name) {
  ...
  var config = cjson.load(path.resolve(".aics/packages.json"));
  var projConf;
  if (!name) {
...
var queryPackage = function(name, version) {
  ...
  var packageJson = cjson.load(path.resolve('.aics/packages.json'));
  //判断依赖的包是否已经存在
    ...
    .on('close', function() {
      fs.createReadStream(tmppath + "/build.zip")
        ...
        .on("close", function() {
          ...
          for (var i = 0; i < trees.length; i++) {
            var file = trees[i];
            if (file.indexOf("build.zip") < 0 && file.indexOf("packages") < 0 && file.indexOf(".DS_Store") < 0) {
...
var exportFiles = function(packages, file, tmppath) {
  ...
  if (fs.existsSync(file)) {
    ...
    Tools.mkdir(file.replace(/\w+\.\w+/, ''));
    fs.createReadStream(tmppath + "/" + file)
