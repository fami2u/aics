...
PackageManager.publish = function(params) {
  ...
  request.post({ url: process.env.AICS_HOST + '/package/verify', body: config, json: true }, function(err, httpResponse, body) {
    ...
    if (body.code > 0) {
      ...
      console.log('编译组件....');
      if (!!body.data) {
        console.log(typeof body.data.pid);
      ...
      buildZip(config, function(path) {
        var r = request.post(process.env.AICS_HOST + '/package/upload', function optionalCallback(err, httpResponse, body) {
          console.log(body.code > 0 ? "发布成功" : '发布失败');
        })
        var form = r.form();
