...
ProjectManager.init = function(args) {
  ...
  if (args.length < 4 || (typeof argv.e == "boolean") || (typeof argv.p == "boolean")) {
    console.log('\n' + chalk.red.bold('参数错误!') + '\n');
    console.log(chalk.green.bold('Usage'));
    console.log("init [option] <string> ");
    console.log('\n' + chalk.green.bold('option:'));
    console.log("[null]              创建aics package项目");
    console.log(" -p                 创建AICS project项目");
    console.log(" -e                 根据项目模版创建项目 访问http://codedepot.fami2u.com/projects 获取更多项目模版");
    console.log('\n' + chalk.green.bold('example:'));
    console.log("aics init fami2u                   创建一个名称为fami2u的代码包");
    console.log("aics init -p fami2u                创建一个名称为fami2u的项目");
    console.log("aics init -e fami2u:example        根据项目模版创建项目 访问http://codedepot.fami2u.com/projects 获取更多项目模版");
    return;
  }
  ...
  if (argv.e) {
    ...
    request(aicsConfig.host + "/project/info?" + qs.stringify(param), function(error, response, body) {
      ...
      fs.stat('.aics', function(err, stats) {
        ...
        fs.writeFile(name + '.md', body.documentation, 'utf8', function(err) {
          if (err) {
...
ProjectManager.add = function(name) {
  ...
  request(aicsConfig.host + "/package/info?" + qs.stringify(param), function(error, response, body) {
    if (body == "NONE") {
      ...
      Tools.setAics(name, result.version);
