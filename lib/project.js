...
ProjectManager.init = function(args) {
    if (args.length < 5) {
        ...
        console.log("init  package  username:packageName                      将当前目录初始化为AICS package项目");
        console.log("init  project --example username:projectName             将当前目录初始化为AICS project项目");
        return;
    }
    var type = args[3];
    if (type == "project") {
        ...
        if (isExample) {
            ...
            fs.stat('.aics', function(err, stats) {
                if (!stats) {
                    fs.mkdir(".aics/tmp", function() {
                        var project = require('../example/aics-project.json');
                        project.name = projectName;
                        fs.writeFile('.aics/aics-project.json', JSON.stringify(project, null, 4), 'utf8', function(err) {
                            if (err) console.log('写入 aics-project.json 错误');
                            fs.writeFile('.aics/package.json', "{}", 'utf8', function(err) {
                                if (err) console.log('写入 package.json 错误');
                            });
                    ...
                    console.log(chalk.red.bold('\n' + ".aics is already exist, command stop.") + '\n');
                }
            })
        }
        //require json 文件
...
ProjectManager.add = function(name) {
    if (!name) {
        ...
        //读取项目配置文件中的
        var config = Tools.projectConfig(".aics/aics-project.json");
        //为当前项目添加包依赖
        var packageinfo = name.split("@");
        config.packages[name] = '';
        fs.writeFile(path.resolve(".aics/aics-project.json"),JSON.stringify(config,null,4),'utf8',function(err,res){
            console.log(err);
            console.log(res);
        });
